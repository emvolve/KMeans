...
def get_distance(query_data, query, p=2):
    ...
    # returns numpy array containing the calculated distances from the query point to each individual feature instance
...
def move_centroids(feature_data, centroid_indices, current_centroids):
    ...
    number_of_features = current_centroids.shape[1]
    number_of_centroids = current_centroids.shape[0]

    new_centroids = np.zeros((number_of_centroids, number_of_features))
    ...
    for i in range(number_of_centroids):
        ...
        # Guard - if a centroid ends up with no assigned points, np.mean of empty array returns NAN
        if len(assigned_points) == 0:
            new_centroids[i] = current_centroids[i] # keep centroid in place
        else:
...
def get_inertia_cost(feature_data, centroid_indices, current_centroids):
    ...
    # Delegate to wcss as wcss is already vectorised
    return wcss(feature_data, centroid_indices, current_centroids)
...
def bcss(feature_data, centroid_indices, current_centroids):
    ...
    dataset_mean = np.mean(feature_data, axis=0).reshape(1, -1)  # explicit 2D reshape for get_distance (was implicit)
...
def restart_KMeans(feature_data, number_of_centroids, iterations, restarts):
    ...
    for i in range(1, restarts + 1):
        ...
        # After all inner iterations, centroid_indices holds the result of the final iteration
        current_iteration_cost = get_inertia_cost(feature_data, centroid_indices, current_centroids)

        if i == 1 or current_iteration_cost < best_cost:
            best_centroids = current_centroids
            best_indices = centroid_indices
